...
> [中文官方网站](https://cn.eslint.org/)
> [英文官方网站](https://eslint.org/)
...
2.  AngularJS
3.  JSX
4.  Style 检查 5.自定义错误和提示
5.  EsLint 提供以下几种校验：

#### 语法错误校验

1.  不重要或丢失的标点符号，如分号
2.  没法运行到的代码块（使用过 WebStorm 的童鞋应该了解）
3.  未被使用的参数提醒
4.  漏掉的结束符，如}
...
2.  文件配置：利用 javascript、json、yml 语言的语法，生成名为.eslintrc.\*的配置文件，将配置信息写到文件内
...
> 规则的错误等级有三种：
> 1.0 或'off'：关闭规则。
> 2.1 或'warn'：打开规则，并且作为一个警告（并不会导致检查不通过）。
> 3.2 或'error'：打开规则，并且作为一个错误 (退出码为 1，检查不通过)。
